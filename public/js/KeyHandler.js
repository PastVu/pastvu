	...
	!function () {
		...
		function handleOpera(handler){
			...
			function holdingStop() {
				holdActive = false;
				delete lastEvent['holdStart']; lastEvent['holdEnd'] = true;
				handler(lastEvent);
				lastEvent = null;
	...
	function handleKey(event) {
	  ...
	  try {
		  ...
		  switch (parseInt(key, 0)) {
			case 38: action = 'onUp'; break;
			case 40: action = 'onDown'; break;
			case 37: action = 'onLeft'; break;
			case 39: action = 'onRight'; break;
			
			case 27: action = 'onEsc'; break;
		  }
		  if(event['holdStart']) action+='HoldStart';
		  if(event['holdEnd']) action+='HoldEnd';
		  
		  var currTarget = keyTarget[keyTarget.length-1];
		  if (currTarget.stopFurther) {
