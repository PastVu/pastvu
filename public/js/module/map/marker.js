	...
	MarkerManager.prototype.refreshDataByZoom = function (init) {
		...
		if (pollServer) {
			...
			socket.once('getBoundsResult', function (data) {
				...
				if (data && !data.error) {
					// Данные устарели и должны быть отброшены, если зум изменился или уже был отправлен другой запрос на данные по зуму или текущий баунд уже успел выйти за пределы запрашиваемого
					if (this.map.getZoom() !== data.z || this.startPendingAt !== data.startAt || !bound.intersects(this.calcBound)) {
	...
	MarkerManager.prototype.refreshDataByMove = function (reposExisting) {
		...
		socket.once('getBoundsResult', function (data) {
			...
			if (data && !data.error) {
				...
				if (this.map.getZoom() !== data.z || !bound.intersects(this.calcBound)) {
