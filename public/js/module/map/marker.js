...
define([
    ...
    MarkerManager.prototype.refreshDataByZoom = function (init) {
        ...
        const willLocalWork = newZoom >= this.firstClientWorkZoom;
        const crossingLocalWorkZoom = this.currZoom < this.firstClientWorkZoom && willLocalWork || this.currZoom >= this.firstClientWorkZoom && !willLocalWork;
        const direction = newZoom - this.currZoom;
        let bound = L.latLngBounds(this.calcBound.getSouthWest(), this.calcBound.getNorthEast());
        let bounds; //Новые баунды для запроса
        let pollServer = true;
        let curr;
        let i;
        ...
        if (pollServer) {
            ...
            socket.run('photo.getByBounds',
                ...
                let localWork; // Находимся ли мы на уровне локальной работы
