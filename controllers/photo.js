...
async function getBounds(data) {
    ...
    if (isPainting === null) {
        // Both photos and paintings mode
        const bothMin = -100, bothMax = 2000;
        ...
        if (!localWork) {
            ({ photos, clusters } = await this.call(
                `cluster.${hasYears ? 'getBothBoundsByYear' : 'getBothBounds'}`,
                data
            ));
        } else {
