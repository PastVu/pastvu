...
export async function configure(startStamp) {
    ...
    if (config.servePublic) {
        ...
        if (env === "development") {
            ...
            app.use(
                "/style",
                lessMiddleware(path.join(pub, "style"), {
                    force: true,
                    once: false,
                    debug: false,
                    render: {
        ...
        app.use(
            require("serve-favicon")(path.join(pub, "favicon.ico"), {
    ...
    if (config.serveStore) {
        ...
        const uploadServer = `http://${
            config.uploader.hostname || "localhost"
        }:${config.uploader.port}`;
        const downloadServer = `http://${
            config.downloader.hostname || "localhost"
        }:${config.downloader.port}`;

        // Serve files for public photos
        app.use(
            "/_p/",
            ourMiddlewares.serveImages(path.join(storePath, "public/photos/"), {
                maxAge: ms("7d"),
            })
        );
        ...
        const prServeMiddleware = ourMiddlewares.serveImages(
            path.join(storePath, "protected/photos/"),
            { maxAge: ms("7d") }
        );

        app.use("/_pr/", async (req, res, next) => {
            try {
                ...
                if (response.statusCode === 303) {
                    // 303 means ok, user can get protected file
        ...
        app.use(
            "/_prn/",
            ourMiddlewares.serveImages(
                path.join(storePath, "publicCovered/photos/"),
                { maxAge: ms("7d") }
            )
        );
        ...
        app.use(
            "/_a/",
            ourMiddlewares.serveImages(
                path.join(storePath, "public/avatars/"),
                { maxAge: ms("2d") }
            )
        );
        ...
        app.use(
            ["/upload", "/uploadava"],
            proxy({ target: uploadServer, logLevel: "warn" })
        );
        app.use(
            "/download",
            proxy({ target: downloadServer, logLevel: "warn" })
        );
    ...
    if (config.serveLog) {
        app.use(
            "/nodelog",
            require("basic-auth-connect")(
                config.serveLogAuth.user,
                config.serveLogAuth.pass
            ),
    ...
    if (manualGC) {
        ...
        logger.info(
            `Manual garbage collection every ${manualGarbageCollect / 1000}s`
        );
    } else {
    ...
    logger.info(
        `servePublic: ${config.servePublic}, serveStore ${config.serveStore}`
    );
    logger.info(`Host for users: [${config.client.host}]`);

    await new CoreServer(
        "Core",
        { port: config.core.port, host: "0.0.0.0" },
        logger
    ).listen();
    ...
    waitDb.then(async () => {
        ...
        listener.addCallback(
            ...
            session.cleanArchivedSessions
        );
        listener.addCallback(
            "calcUserStats",
            session.regetUsersAfterStatsUpdate
        );
        listener.init();
